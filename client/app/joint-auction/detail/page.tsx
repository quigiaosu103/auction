...
import { JointAuction } from "@/app/@types/JointAuction.type";
...
export default function page() {
  ...
  const [transactionsOfAuction, setTransactionOfAuction] = useState<
    BidTransaction[]
  >([]);
  const [jointAuction, setJointAuction] = useState<JointAuction | null>(null);
    ...
    const getData = async () => {
      if (wallet) {
        const result = await wallet.viewMethod({
          ...
          method: "get_joint_auction_metadata_by_joint_auction_id",
          args: {
            joint_auction_id: id,
          },
        });
  ...
  const bidJointAuction = async (e: any) => {
    if (!wallet) {
    ...
    await wallet
      .callMethod({
        contractId: CONTRACT_ID,
        method: "bid_joint_auction",
        args: { joint_auction_id: id },
  ...
  const finishJointAuction = async (e: any) => {
    if (!wallet) {
      console.error("Wallet is not initialized");
      return;
    }
